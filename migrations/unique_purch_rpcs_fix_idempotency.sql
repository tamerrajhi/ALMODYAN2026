-- ============================================================
-- FIX: Adjust ON CONFLICT to match atomic_workflow_requests PK
-- The table PK is (client_request_id) only, not composite
-- Strategy: append workflow_type to client_request_id to create unique keys
-- ============================================================

-- Re-create import_excel_atomic with fixed idempotency
...
DECLARE
  ...
  v_wf_key        text  := v_client_req_id || '::unique_import_excel';
  v_branch_code   text;
  v_batch_id      uuid;
  v_batch_no      text;
  v_total_items   int := 0;
...
-- Re-create return_create_atomic with fixed idempotency
...
DECLARE
  ...
  v_wf_key         text  := v_client_req_id || '::unique_purchase_return';
  v_branch_code    text;
  v_return_id      uuid;
  v_return_no      text;
...
-- Re-create return_void_atomic with fixed idempotency
