-- ============================================================
-- PATCH: NULLIF for empty-string numeric casts + Stone/Metal/M text fix
-- ============================================================
-- Discovered during STOP-2 real Excel import (Purchase 1 SUPP.xlsx).
-- Two issues:
--   1) Empty string "" fails on ::numeric cast. COALESCE only catches NULL.
--      Fix: NULLIF(val, '') converts "" to NULL before ::numeric.
--   2) Excel columns "Stone" (col 22), "Metal" (col 23), "M" (col 24) contain
--      text codes ("D", "G", "M"), NOT numeric weights.
--      Fix: mq_weight/cs_weight/stone_weight/metal_weight/m_weight = 0 (no numeric source).
--      Text fields: metal, stone, rate_type, clarity use case-sensitive header keys.
-- ============================================================
...
BEGIN
  ...
  LOOP
    ...
    -- vat_rate is decimal (0.15), NOT percentage (15). No /100.
    ...
    LOOP
      ...
      INSERT INTO unique_items (
        ...
        0, -- mq_weight: no numeric source in Excel
        0, -- cs_weight: no numeric source in Excel
        0, -- stone_weight: Excel "Stone" col is text code, not weight
        0, -- metal_weight: Excel "Metal" col is text code, not weight
        0, -- m_weight: Excel "M" col is text code, not weight
