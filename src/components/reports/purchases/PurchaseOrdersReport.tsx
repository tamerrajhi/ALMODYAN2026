...
export default function PurchaseOrdersReport({ onBack }: PurchaseOrdersReportProps) {
  ...
  const { data: orders, isLoading } = useQuery({
    queryKey: ['purchase-orders-report', dateFrom, dateTo],
    queryFn: async () => {
      const params = new URLSearchParams();
      if (dateFrom) params.set('startDate', format(dateFrom, 'yyyy-MM-dd'));
      if (dateTo) params.set('endDate', format(dateTo, 'yyyy-MM-dd'));
      const res = await fetch(`/api/reports/purchase-orders-list?${params}`, { credentials: 'include' });
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold">{language === 'ar' ? 'تقرير أوامر الشراء' : 'Purchase Orders Report'}</h2>
            <p className="text-muted-foreground text-sm">{language === 'ar' ? 'تحليل أوامر الشراء والموردين' : 'Purchase orders and suppliers analysis'}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">{language === 'ar' ? 'إجمالي الأوامر' : 'Total Orders'}</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">{language === 'ar' ? 'إجمالي المبالغ' : 'Total Amount'}</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{totalAmount.toLocaleString()} {t.currency.sar}</div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">{language === 'ar' ? 'أوامر معلقة' : 'Pending Orders'}</CardTitle>
          </CardHeader>
          <CardContent>
