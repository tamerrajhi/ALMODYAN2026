-- =============================================================================
-- HOTFIX (STRICT SCOPE): Make unique purchase return totals robust to item lookup
--
-- Problem observed in UI/network logs:
-- Payload contains unit_price > 0, yet v_subtotal computed as 0 because the
-- previous implementation only added to subtotal when the jewelry_items row
-- was found (v_item_record IS NOT NULL). In any case where lookup returns 0
-- rows (RLS/filtering, data drift), subtotal stayed 0 and the fail-fast guard
-- triggered.
--
-- Fix (NO schema changes, NO payload changes):
-- 1) Always derive v_unit_price from payload when provided.
-- 2) If payload unit_price missing/0, fallback to jewelry_items.cost.
-- 3) If item not found OR computed unit_price <= 0 => raise explicit exception.
-- 4) Keep purchase_invoice_lines insert using subtotal/total_amount only.
-- =============================================================================
...
BEGIN
    ...
    -- Robust mapping:
    -- - Prefer payload unit_price when provided
    -- - Else fallback to jewelry_items.cost
    -- - If still <= 0 => fail fast with item-specific message
    -- ================================
    ...
    -- Create return header (invoice_type = purchase_return)
    ...
    -- Create return lines + update items
    v_line_number := 0;
    ...
    LOOP
        ...
        -- Ensure item exists for status updates; price prefers payload
        SELECT * INTO v_item_record FROM jewelry_items WHERE id = v_item_id;
        IF v_item_record IS NULL THEN
    ...
    -- Journal entry (must use generator)
