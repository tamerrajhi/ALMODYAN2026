-- =====================================================
-- PATCH: Fix missing full_name in auth.users
-- Target: complete_purchase_return_unique_items_atomic
-- Uses raw_user_meta_data->>'full_name' instead of non-existent full_name column
-- =====================================================
...
BEGIN
    ...
    -- ================================
    -- FIX: User name resolution using raw_user_meta_data
    -- ================================
    v_user_id := auth.uid();
    SELECT COALESCE(
    ...
    LOOP
        ...
        IF v_item_record IS NOT NULL THEN
            ...
            -- Update jewelry item status to returned (FIXED: use sale_status, not status)
