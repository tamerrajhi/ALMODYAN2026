...
-- Steps 1-4: RLS Fix + Void Columns + Posted Lock + Void RPC
-- =====================================================================
  ...
  IF OLD.journal_entry_id IS NOT NULL THEN
    ...
    IF v_je_posted = true THEN
      ...
      IF NEW.status = 'voided' AND OLD.status != 'voided' THEN
        -- This is allowed (void operation)
        RETURN NEW;
      END IF;
      ...
      THEN
        -- Check that financial fields are not changed
        IF NEW.total_amount = OLD.total_amount
           AND NEW.subtotal = OLD.subtotal
           AND NEW.tax_amount = OLD.tax_amount
           AND NEW.customer_id IS NOT DISTINCT FROM OLD.customer_id
...
BEGIN
  ...
  IF v_invoice.journal_entry_id IS NOT NULL THEN
    ...
    IF v_original_je IS NOT NULL AND v_original_je.is_posted = true THEN
      ...
      VALUES (
        ...
        v_original_je.total_credit,  -- swap
        v_original_je.total_debit,   -- swap
        'posted',
        true,
        v_invoice.branch_id,
        v_user_id,
      ...
      SELECT
        v_reversal_je_id,
        jel.account_id,
        jel.credit_amount,  -- swap: old credit becomes new debit
        jel.debit_amount,   -- swap: old debit becomes new credit
  ...
  LOOP
    ...
    -- Record void movement in finished_goods_movements
