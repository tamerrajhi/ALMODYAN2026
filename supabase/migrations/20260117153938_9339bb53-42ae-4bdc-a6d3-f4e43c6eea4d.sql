-- ============================================================================
-- Atomic RPC: convert_prs_to_pos_atomic
-- Converts Purchase Requisitions to Purchase Orders atomically with:
-- 1. Idempotency via atomic_workflow_requests
-- 2. Row-level locking (FOR UPDATE) on PRs and PR items
-- 3. Multi-PO creation grouped by supplier
-- ============================================================================
...
BEGIN
  -- ========================================================================
  -- B2-7.2: IDEMPOTENCY CHECK
  -- ========================================================================
  v_client_request_id := p_payload->>'client_request_id';
  
  IF v_client_request_id IS NULL OR v_client_request_id = '' THEN
    RETURN jsonb_build_object('success', false, 'error', 'VALIDATION: client_request_id is required');
  END IF;
  ...
  IF FOUND THEN
    ...
    ELSIF v_existing_request.status = 'in_progress' THEN
      RETURN jsonb_build_object('success', false, 'error', 'IN_PROGRESS: request is already processing');
    ELSE
      -- Failed before, allow retry by updating to in_progress
  ...
  IF v_pr_ids IS NULL OR array_length(v_pr_ids, 1) = 0 THEN
    UPDATE public.atomic_workflow_requests
    SET status = 'failed', error_message = 'VALIDATION: pr_ids is required', completed_at = now()
    WHERE client_request_id = v_client_request_id;
    RETURN jsonb_build_object('success', false, 'error', 'VALIDATION: pr_ids is required');
  END IF;
  
  IF v_items IS NULL OR jsonb_array_length(v_items) = 0 THEN
    UPDATE public.atomic_workflow_requests
    SET status = 'failed', error_message = 'VALIDATION: items is required', completed_at = now()
    WHERE client_request_id = v_client_request_id;
    RETURN jsonb_build_object('success', false, 'error', 'VALIDATION: items is required');
  END IF;
  ...
  LOOP
    ...
    IF v_pr_record.status NOT IN ('approved', 'partially_converted') THEN
      UPDATE public.atomic_workflow_requests
      SET status = 'failed', 
          error_message = format('VALIDATION: PR %s has invalid status: %s', v_pr_record.requisition_number, v_pr_record.status),
          completed_at = now()
      WHERE client_request_id = v_client_request_id;
      RETURN jsonb_build_object('success', false, 'error', 
        format('طلب الشراء %s في حالة غير مسموحة: %s', v_pr_record.requisition_number, v_pr_record.status));
    END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM public.purchase_requisitions WHERE id = ANY(v_pr_ids)) THEN
    UPDATE public.atomic_workflow_requests
    SET status = 'failed', error_message = 'VALIDATION: No PRs found with given IDs', completed_at = now()
    WHERE client_request_id = v_client_request_id;
    RETURN jsonb_build_object('success', false, 'error', 'VALIDATION: No PRs found with given IDs');
  END IF;
  ...
  LOOP
    ...
    IF v_item_supplier_id IS NULL THEN
      UPDATE public.atomic_workflow_requests
      SET status = 'failed', error_message = 'VALIDATION: All items must have a supplier', completed_at = now()
      WHERE client_request_id = v_client_request_id;
      RETURN jsonb_build_object('success', false, 'error', 'برجاء اختيار مورد لأمر الشراء أو لكل بند قبل الحفظ');
    END IF;
    ...
    IF NOT FOUND THEN
      UPDATE public.atomic_workflow_requests
      SET status = 'failed', error_message = format('VALIDATION: PR item not found: %s', v_item_pr_item_id), completed_at = now()
      WHERE client_request_id = v_client_request_id;
      RETURN jsonb_build_object('success', false, 'error', format('بند طلب الشراء غير موجود: %s', v_item_pr_item_id));
    END IF;
    
    -- Validate item belongs to one of the PRs
    IF NOT (v_pr_item.requisition_id = ANY(v_pr_ids)) THEN
      UPDATE public.atomic_workflow_requests
      SET status = 'failed', error_message = 'VALIDATION: Item does not belong to selected PRs', completed_at = now()
      WHERE client_request_id = v_client_request_id;
      RETURN jsonb_build_object('success', false, 'error', 'البند لا ينتمي لطلبات الشراء المحددة');
    END IF;
    ...
    IF v_item_convert_qty > (v_pr_item.quantity - v_pr_item.converted_quantity) THEN
      UPDATE public.atomic_workflow_requests
      SET status = 'failed', 
          error_message = format('VALIDATION: Convert quantity exceeds available for item %s', v_pr_item.item_description),
          completed_at = now()
      WHERE client_request_id = v_client_request_id;
      RETURN jsonb_build_object('success', false, 'error', 
        format('الكمية المطلوب تحويلها (%s) أكبر من المتاح (%s) للبند: %s', 
  ...
  IF v_supplier_groups = '{}'::jsonb THEN
    UPDATE public.atomic_workflow_requests
    SET status = 'failed', error_message = 'VALIDATION: No items to convert', completed_at = now()
    WHERE client_request_id = v_client_request_id;
    RETURN jsonb_build_object('success', false, 'error', 'الرجاء تحديد بند واحد على الأقل');
  END IF;
...
EXCEPTION WHEN OTHERS THEN
  ...
  RETURN jsonb_build_object('success', false, 'error', 'DB_ERROR: ' || SQLERRM);
END;
$$;
