...
-- Uses canonical workflow writers instead of direct writes to atomic_workflow_requests
-- Tables: payments, supplier_payment_allocations
-- Journal: reference_type='payment_voucher', is_posted=true
-- =============================================================================
...
BEGIN
  ...
  IF v_payment_type = 'payment' 
    ...
    PERFORM public.core_workflow_failed(
      ...
      v_workflow_type,
      'SET_HB',
      'Supplier payment requires allocations or allow_unallocated=true'
    );
...
-- Comment documenting PV-3 + canonical writers
COMMENT ON FUNCTION public.payment_voucher_atomic(jsonb) IS 
'PV-3 Stage-1 Canonical Restore: Uses begin_workflow_request/core_workflow_success/core_workflow_failed instead of direct writes to atomic_workflow_requests. Creates payments + supplier_payment_allocations + journal_entries with reference_type=payment_voucher and is_posted=true.';