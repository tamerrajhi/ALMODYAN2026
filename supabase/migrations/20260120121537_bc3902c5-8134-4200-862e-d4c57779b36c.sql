...
-- ═══════════════════════════════════════════════════════════════════════════════
-- FIX-P1.2: Stabilize payment_voucher_atomic
-- Addresses:
--   P1.2-01: Move Hard Block after begin_workflow_request + log failure
--   P1.2-02: Use cached_result from begin_workflow_request (no SELECT from pos_workflow_requests)
--   P1.2-03: Supplier Match validation for allocations
--   P1.2-04: Locking consistency (FOR UPDATE in update loop - already present)
--   P1.2-05: Preserve core_workflow_success(uuid, uuid, jsonb) 3-arg signature
--   P1.2-06: Grants remain authenticated only
-- ═══════════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  IF v_begin_status = 'succeeded' THEN
    -- P1.2-02: Return cached_result directly from begin_workflow_request
    -- NO SELECT from pos_workflow_requests
    RETURN COALESCE(
  ...
  -- P1.2-01: Log failure to ledger before returning
  -- ═══════════════════════════════════════════════════════════════
  ...
  -- P1.2-03: Include Supplier Match validation
  -- ═══════════════════════════════════════════════════════════════
  IF v_allocations IS NOT NULL AND jsonb_array_length(v_allocations) > 0 THEN
    ...
    LOOP
      ...
      -- P1.2-03: Supplier Match validation
      IF v_payment_type = 'payment' AND v_supplier_id IS NOT NULL THEN
  ...
  BEGIN
    ...
    -- P1.2-03: Re-validate supplier match in update loop
    -- P1.2-04: FOR UPDATE locking on invoice row
    -- ═══════════════════════════════════════════════════════════════
    IF v_allocations IS NOT NULL AND jsonb_array_length(v_allocations) > 0 THEN
      FOR v_alloc IN SELECT * FROM jsonb_array_elements(v_allocations)
      LOOP
        ...
        -- P1.2-04: FOR UPDATE: Lock invoice row for atomic update
        SELECT 
          COALESCE(total_amount, 0),
          COALESCE(total_returned_amount, 0),
          COALESCE(paid_amount, 0),
        ...
        -- P1.2-03: Re-validate supplier match (defense in depth)
        IF v_payment_type = 'payment' AND v_supplier_id IS NOT NULL THEN
          IF v_invoice_supplier_id IS DISTINCT FROM v_supplier_id THEN
    ...
    -- P1.2-05: core_workflow_success with 3 args (uuid, uuid, jsonb)
...
-- ═══════════════════════════════════════════════════════════════
-- P1.2-06: Ensure grants remain authenticated only
-- ═══════════════════════════════════════════════════════════════
...
'Atomic Payment Voucher creation with allocations support.
FIX-P1.2 (2026-01-20):
- P1.2-01: Hard Block moved after begin_workflow_request, failures logged to ledger
- P1.2-02: Cached result returned directly from begin_workflow_request (no pos_workflow_requests SELECT)
- P1.2-03: Supplier Match validation prevents cross-supplier allocation
- P1.2-04: FOR UPDATE locking in update loop
- P1.2-05: core_workflow_success(uuid, uuid, jsonb) 3-arg signature
- P1.2-06: Grants restricted to authenticated only';
