-- =====================================================
-- B2-10T: Idempotency Smoke Test RPC
-- Uses ONLY canonical workflow functions
-- NO direct writes to atomic_workflow_requests
-- =====================================================
...
BEGIN
  ...
  IF v_client_request_id IS NULL OR v_client_request_id = '' THEN
    -- Return validation error WITHOUT calling begin_workflow_request
  ...
  -- Calculate payload hash for meta (canonical function handles conflict detection)
  ...
  -- CONFLICT: Same client_request_id but different payload
  IF v_begin_result->>'status' = 'CONFLICT' THEN
    RETURN jsonb_build_object(
      'success', false,
  ...
  -- Any other unexpected status from begin_workflow_request
  IF v_begin_result->>'status' NOT IN ('OK', 'RETRY') THEN
    RETURN jsonb_build_object(
      'success', false,
      'error_code', 'WORKFLOW_ERROR',
      'error', 'Unexpected workflow status: ' || COALESCE(v_begin_result->>'status', 'null')
    );
  END IF;
  
  -- ===========================================
  -- 4) Execute "business logic" (optional sleep for IN_PROGRESS testing)
  -- ===========================================
...
EXCEPTION WHEN OTHERS THEN
  -- ===========================================
  -- Error handling: use canonical failure function
  -- ===========================================
  BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN
    -- Ignore nested exception (fail gracefully)
    NULL;
  END;
...
GRANT EXECUTE ON FUNCTION public.idempotency_smoke_atomic(jsonb) TO authenticated;
GRANT EXECUTE ON FUNCTION public.idempotency_smoke_atomic(jsonb) TO service_role;

-- Add comment for documentation
COMMENT ON FUNCTION public.idempotency_smoke_atomic(jsonb) IS 
'Idempotency smoke test RPC - uses only canonical workflow functions (begin_workflow_request, core_workflow_success, core_workflow_failed). No direct writes to atomic_workflow_requests.';