-- Phase A: Add Journal Entry creation to create_transfer_v2
-- Condition: Creates JE only when total_cost > 0 AND inventory accounts exist for both branches
...
BEGIN
  ...
  -- LOCKING (FOR UPDATE)
  -- ===================
  -- Lock all jewelry_items rows to prevent concurrent modifications
  ...
  FOR v_item IN 
    ...
    WHERE id = ANY(v_item_ids)
  LOOP
    -- Check branch matches from_branch_id
    ...
    -- Check sale_status is available or null
    ...
    -- If invoice mode, check purchase_invoice_id matches
  ...
  -- ===================
  -- Use to_branch_id for code sequence
  v_seq_branch_id := v_to_branch_id;
  
  -- Get or create sequence for TRF
  ...
  IF NOT FOUND THEN
    -- Get branch code to create prefix
  ...
  -- ===================
  -- CALCULATE TOTALS FROM DB (BEFORE movements for accurate cost)
  -- ===================
  ...
  IF v_total_cost > 0 THEN
    -- Get source branch inventory account (from_branch)
    ...
    -- Get target branch inventory account (to_branch)
    SELECT COALESCE(general_inventory_account_id, imported_pieces_account_id)
    ...
    IF v_source_account_id IS NOT NULL AND v_target_account_id IS NOT NULL THEN
      -- Generate journal entry number using daily sequence (JE-YYYYMMDD-NNNN)
      ...
      -- Line 1: Debit target branch inventory (receiving)
      ...
      VALUES (
        ...
        v_total_cost,
        0,
        'مخزون وارد - ' || v_transfer_code,
        now()
      );
      
      -- Line 2: Credit source branch inventory (sending)
      INSERT INTO public.journal_entry_lines (
        id,
        journal_entry_id,
        account_id,
