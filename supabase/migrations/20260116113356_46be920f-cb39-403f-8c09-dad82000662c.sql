...
BEGIN
  ...
  SELECT public.generate_document_code('TRF') INTO v_transfer_number;
  
  -- Create transfer header
  INSERT INTO public.transfers (
    id, transfer_number, from_branch_id, to_branch_id, 
    status, notes, purchase_invoice_id, transfer_date, created_at
  ) VALUES (
    gen_random_uuid(), v_transfer_number, v_from_branch_id, v_to_branch_id,
    'completed', v_notes, v_purchase_invoice_id, now(), now()
  )
  ...
  IF v_from_account_id IS NOT NULL AND v_to_account_id IS NOT NULL AND v_total_value > 0 THEN
    ...
    INSERT INTO public.journal_entries (
      ...
      'قيد نقل مخزون - ' || v_transfer_number,
