-- =============================================================================
-- PURCH-PR2 STEP 2: Fix Unique Purchase Return - Add branch_id NULL + item_movements
--
-- Problems fixed:
-- 1. jewelry_items.branch_id not set to NULL on confirmed return
-- 2. No item_movements record created for purchase return
--
-- Solution:
-- 1. Update jewelry_items SET branch_id = NULL (item exits branch)
-- 2. Insert item_movements with movement_type = 'PURCHASE_RETURN', idempotent-safe
-- 3. Post-condition guard to ensure movements match items
-- =============================================================================
...
BEGIN
    ...
    -- + UPDATE jewelry_items (status + branch_id = NULL)
    -- + INSERT item_movements (PURCHASE_RETURN)
    -- ================================
    FOR v_item IN SELECT * FROM jsonb_array_elements(v_items)
    LOOP
        v_item_id := COALESCE((v_item->>'item_id')::UUID, (v_item->>'id')::UUID);
    ...
    IF v_actual_movement_count <> v_expected_movement_count THEN
        RAISE EXCEPTION 'PURCHASE_RETURN_MOVEMENTS_INCOMPLETE: expected=%, actual=%', 
            v_expected_movement_count, v_actual_movement_count;
    END IF;
    ...
    RETURN jsonb_build_object(
        ...
        'movements_created', v_actual_movement_count
    );
