-- =========================================
-- P-ACC-CONFIG-02-SEED + P-ACC-CONFIG-03-REFACTOR
-- Seed Global Defaults + Fix RPC
-- =========================================

-- =========================================
-- PHASE C: SEED Global Defaults
-- =========================================

-- C1) Insert Global Defaults (branch_id = NULL)
INSERT INTO public.branch_accounting_config (branch_id, config_key, account_id, is_active)
SELECT 
  NULL,
  k.key,
  coa.id,
...
FROM (
  VALUES 
    ('inventory', '1301'),
    ('vat_input', '2105'),  -- Using 2105 since 1501 doesn't exist
    ('ap_supplier', '2101')
) AS k(key, code)
JOIN public.chart_of_accounts coa ON coa.account_code = k.code AND coa.is_active = true
ON CONFLICT (branch_id, config_key) DO UPDATE SET
  account_id = EXCLUDED.account_id,
  updated_at = now();

-- =========================================
-- PHASE D: REFACTOR RPC (No Hardcode!)
-- =========================================
...
BEGIN
  ...
  -- =========================
  -- 4.1) Inventory: REQUIRED - use dynamic config
  v_inventory_account_id := get_branch_account_id(v_branch_id, 'inventory');
  
  -- 4.2) VAT Input: OPTIONAL - only required if tax > 0
  v_vat_input_account_id := get_branch_account_id(v_branch_id, 'vat_input');
  
  -- 4.3) Supplier Account: prefer supplier.account_id, fallback to config
  BEGIN
  ...
  LOOP
    ...
    INSERT INTO public.item_movements (
      id, jewelry_item_id, movement_type,
      from_branch_id, to_branch_id,
      movement_date, performed_by, notes,
      purchase_return_id, created_at
    ) VALUES (
  ...
  IF v_tax_amount > 0 THEN
    IF v_vat_input_account_id IS NULL THEN
      -- VAT is required but no account configured - FAIL
      PERFORM pos_fail_request(v_client_request_id, 'CONFIG_ERROR', 
