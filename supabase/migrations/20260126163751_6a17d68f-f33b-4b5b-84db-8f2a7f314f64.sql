-- ============================================================
-- v_returns_hub: Canonical Returns Hub Read Model
-- ============================================================

-- Create/Replace View (final canonical-safe version)
...
-- Permissions
GRANT SELECT ON public.v_returns_hub TO authenticated;

-- Documentation
COMMENT ON VIEW public.v_returns_hub IS
'Canonical rules:
1) purchase_returns.return_number => UNIQUE canonical row (one row per return_number).
2) invoices invoice_type=purchase_return with lines=0 and matching purchase_returns => MIRROR (never surfaces as a separate row; mirror_exists=true on unique row).
3) invoices invoice_type=purchase_return with lines>0 and no matching purchase_returns => GENERAL canonical row.
Drift fields:
- unique: has_drift true when movement_count mismatch OR branch_not_cleared.
- general: drift fields are NULL.';