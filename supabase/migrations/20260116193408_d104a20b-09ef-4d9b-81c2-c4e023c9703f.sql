-- Fix column name: item_id for item_movements
...
BEGIN
  ...
  INSERT INTO public.transfers (
    ...
    notes, purchase_invoice_id, status, total_items, total_cost, created_at
  ) VALUES (
    v_transfer_id, v_transfer_code, v_from_branch_id, v_to_branch_id, v_transfer_date,
    v_notes, v_purchase_invoice_id, 'posted', v_total_items, v_total_cost, now()
  );

  -- INSERT TRANSFER ITEMS (FIXED: item_id)
  INSERT INTO public.transfer_items (transfer_id, item_id, item_code, weight_grams, unit_cost)
  SELECT v_transfer_id, ji.id, ji.item_code, ji.g_weight, ji.cost
  ...
  IF v_total_cost > 0 THEN
    ...
    INSERT INTO public.journal_entries (
      ...
      description, total_debit, total_credit, is_posted, branch_id, created_at
    ) VALUES (
      v_journal_entry_id, v_journal_entry_number, v_transfer_date::date, 'transfer', v_transfer_id,
      'قيد نقل مخزون - ' || v_transfer_code, v_total_cost, v_total_cost, true, v_to_branch_id, now()
    );
    
    INSERT INTO public.journal_entry_lines (journal_entry_id, account_id, debit_amount, credit_amount, description)
    VALUES (v_journal_entry_id, v_target_account_id, v_total_cost, 0, 'مدين - مخزون الفرع المستلم');
    
    INSERT INTO public.journal_entry_lines (journal_entry_id, account_id, debit_amount, credit_amount, description)
    VALUES (v_journal_entry_id, COALESCE(v_source_account_id, v_target_account_id), 0, v_total_cost, 'دائن - مخزون الفرع المصدر');
    
    UPDATE public.transfers SET journal_entry_id = v_journal_entry_id WHERE id = v_transfer_id;
  END IF;

  -- CREATE ITEM MOVEMENTS (FIXED: item_id)
  INSERT INTO public.item_movements (
    item_id, movement_type, from_branch_id, to_branch_id,
    reference_id, reference_type, notes, performed_by, journal_entry_id, created_at
  )
  SELECT 
    ji.id, 'transfer', v_from_branch_id, v_to_branch_id,
    v_transfer_id, 'transfer', v_notes, auth.uid()::text, v_journal_entry_id, now()
  FROM public.jewelry_items ji
  WHERE ji.id = ANY(v_item_ids);
