...
-- ============================================================================
-- P6-2A: Void/Reverse Governance Production Fix
-- ============================================================================
-- Fixes:
--   G1: Add void metadata columns to purchase_returns
--   G2: Create audit_events table for critical action tracking
--   G3: Update void_purchase_return_atomic to handle:
--       - Unique returns (purchase_returns) 
--       - General returns (invoice-based)
--       - Invoice mirrors (cancel on unique void)
--       - Audit trail logging
-- ============================================================================

-- ============================================================================
-- A1) Add void metadata to purchase_returns
-- ============================================================================
...
-- ============================================================================
-- A2) Create dedicated audit_events table for critical actions
-- ============================================================================
...
-- Admin-only insert (system inserts via SECURITY DEFINER functions)
...
-- ============================================================================
-- A3) Backfill: cancel mirrors for already-voided unique returns
-- Fixed syntax: use subquery approach for Postgres UPDATE with multiple joins
-- ============================================================================
UPDATE invoices
SET
  status = 'cancelled',
  voided_at = COALESCE(invoices.voided_at, now()),
  void_reason = COALESCE(invoices.void_reason, 'Backfill: mirror cancelled due to voided unique return')
WHERE id IN (
  SELECT inv.id
  FROM invoices inv
  LEFT JOIN purchase_invoice_lines pil ON pil.invoice_id = inv.id
  JOIN purchase_returns pr ON pr.return_number = inv.invoice_number
  WHERE inv.invoice_type = 'purchase_return'
    AND pr.status IN ('voided', 'cancelled')
...
-- ============================================================================
-- B) Enhanced void_purchase_return_atomic RPC
-- ============================================================================
-- Supports:
--   1. Unique returns (purchase_returns table)
--   2. General returns (invoice-based with invoice_type='purchase_return')
--   3. Invoice mirror cancellation on unique void
...
BEGIN
  ...
  IF v_is_unique THEN
    ...
    -- Update purchase_returns status with void metadata (G1 fix)
    ...
    LOOP
      ...
      -- Insert reversal movement (idempotent via ON CONFLICT)
    ...
    -- Update Invoice Mirror to cancelled (G2/V7 fix)
