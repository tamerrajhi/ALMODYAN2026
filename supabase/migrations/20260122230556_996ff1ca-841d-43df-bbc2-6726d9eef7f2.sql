...
-- Fix purchase_order_receive_v2_atomic to handle received_by properly
...
BEGIN
  ...
  BEGIN
    ...
    FOR v_receipt IN SELECT * FROM jsonb_array_elements(v_receipts) LOOP
      ...
      -- Get PO item details and lock
      SELECT 
        item_type, description, gold_karat_id, gemstone_type_id,
        quantity, weight_grams, unit_price,
        COALESCE(received_qty, 0), COALESCE(received_weight, 0)
      INTO 
      ...
      INSERT INTO goods_receipt_items (
        grn_id, po_item_id, item_type, description, gold_karat_id, gemstone_type_id,
    ...
    WHERE po_id = v_order_id
      AND (
        ...
        (quantity > 0 AND COALESCE(received_qty, 0) < quantity)
      );
