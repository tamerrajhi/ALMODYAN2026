-- =====================================================
-- P-PURCH-D2-5: FIX VOID FOR GENERAL RETURNS (CANONICAL)
-- Fix General returns to use purchase_returns table instead of invoices
-- =====================================================
...
BEGIN
  ...
  -- (Evidence shows general returns have 0 movements, but we handle it safely)
  -- ============================
  IF v_return_type = 'general' THEN
    ...
    -- Also cancel/void any legacy mirror invoice (if exists)
    v_mirror_invoice_id := NULL;
    SELECT id INTO v_mirror_invoice_id
    FROM public.invoices inv
    WHERE inv.invoice_type = 'purchase_return'
      AND inv.purchase_type = 'general'
      AND inv.journal_entry_id = v_return_rec.journal_entry_id
    ...
    IF v_mirror_invoice_id IS NOT NULL THEN
      ...
      SET 
        ...
        voided_at = now(),
        voided_by = v_voided_by_uuid,
        void_reason = '[D2-5] Canonical general return voided: ' || v_void_reason,
        updated_at = now()
      WHERE id = v_mirror_invoice_id;
    END IF;
  END IF;
...
COMMENT ON FUNCTION public.void_purchase_return_atomic(jsonb) IS 
  'D2-5: Canonical void for both Unique and General purchase returns. 
   Both types now resolve from purchase_returns table. 
   General returns no longer use invoices table (legacy mirrors deprecated in D2-3/D2-4).';