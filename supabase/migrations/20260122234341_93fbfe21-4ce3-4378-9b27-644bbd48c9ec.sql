-- =============================================================================
-- P3-3A Receiving Hotfix Governance Migration
-- Date: 2026-01-22
-- Purpose: Codify all DB hotfixes applied during receiving operational testing
-- =============================================================================

-- -----------------------------------------------------------------------------
-- A) Ensure gold_vault_transactions constraints are properly defined
-- -----------------------------------------------------------------------------

-- Drop existing constraints if they exist, then recreate with stable names
DO $$ 
BEGIN
  -- Drop reference_type constraint if exists
  IF EXISTS (
    SELECT 1 FROM pg_constraint c
    JOIN pg_class t ON t.oid = c.conrelid
    JOIN pg_namespace n ON n.oid = t.relnamespace
    WHERE n.nspname = 'public' 
      AND t.relname = 'gold_vault_transactions'
      AND c.conname = 'gold_vault_transactions_reference_type_check'
  ) THEN
    ALTER TABLE public.gold_vault_transactions DROP CONSTRAINT gold_vault_transactions_reference_type_check;
  END IF;

  -- Drop transaction_type constraint if exists  
  IF EXISTS (
    SELECT 1 FROM pg_constraint c
    JOIN pg_class t ON t.oid = c.conrelid
    JOIN pg_namespace n ON n.oid = t.relnamespace
    WHERE n.nspname = 'public' 
      AND t.relname = 'gold_vault_transactions'
      AND c.conname = 'gold_vault_transactions_transaction_type_check'
  ) THEN
    ALTER TABLE public.gold_vault_transactions DROP CONSTRAINT gold_vault_transactions_transaction_type_check;
  END IF;
END $$;

-- Add reference_type constraint with 'goods_receipt' included
ALTER TABLE public.gold_vault_transactions 
...
-- Add transaction_type constraint with 'receive' (not 'deposit')
ALTER TABLE public.gold_vault_transactions 
ADD CONSTRAINT gold_vault_transactions_transaction_type_check 
...
-- -----------------------------------------------------------------------------
-- B) CREATE OR REPLACE purchase_order_receive_v2_atomic (Final Version)
-- -----------------------------------------------------------------------------
-- Changes from original:
--   1) Column references: karat_id (not gold_karat_id)
--   2) Column references: received_quantity/received_weight (not received_qty)
--   3) transaction_type: 'receive' (not 'deposit')
--   4) reference_type: 'goods_receipt' (newly allowed)
--   5) Accepts both po_item_id and item_id in receipts array
-- -----------------------------------------------------------------------------
...
BEGIN
  ...
  BEGIN
    ...
    FOR v_receipt IN SELECT * FROM jsonb_array_elements(v_receipts) LOOP
      -- Accept both po_item_id and item_id for flexibility
      ...
      -- Use correct column names: karat_id, received_quantity, received_weight
      SELECT 
      ...
      -- Use correct column name: karat_id (not gold_karat_id), total_amount (not total_price)
      ...
      -- Update PO item with correct column names: received_quantity, received_weight
      ...
      -- Gold vault transaction - use 'receive' (not 'deposit'), reference_type 'goods_receipt'
...
'P3-3A Governed Version (2026-01-22): Atomic PO receiving with GRN creation, inventory effects, and proper constraint alignment.';