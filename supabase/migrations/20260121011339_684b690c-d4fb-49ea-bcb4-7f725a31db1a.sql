-- Add Advisory Lock to complete_purchase_return_unique_items_atomic for full safety
...
BEGIN
  ...
  END IF;
  -- status = 'ok' or 'retry' => proceed with execution
  ...
  -- ============================
  -- 8. Create Purchase Return Header
  -- ============================
  ...
  -- ============================
  -- 9. Create Return Items + Movements + Update Jewelry
  -- ============================
  FOR v_item IN SELECT * FROM jsonb_array_elements(v_items)
  LOOP
    v_jewelry_item_id := (v_item->>'item_id')::uuid;
    ...
    IF v_jewelry_item_id IS NOT NULL THEN
      ...
      -- Update jewelry item status (HOTFIX: using 'returned' to match CHECK constraint)
      UPDATE public.jewelry_items SET sale_status = 'returned', branch_id = NULL, updated_at = NOW() WHERE id = v_jewelry_item_id;
    END IF;
  END LOOP;
...
EXCEPTION WHEN OTHERS THEN
  -- ============================
  -- 11. Handle Failure & Mark Workflow as Failed
  -- ============================
  BEGIN
    PERFORM public.fail_workflow_request(v_client_request_id, SQLSTATE, SQLERRM);
  EXCEPTION WHEN OTHERS THEN
    -- Ignore failure in fail_workflow_request itself
    NULL;
  END;
