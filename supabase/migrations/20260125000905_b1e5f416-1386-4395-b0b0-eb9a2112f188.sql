...
-- =====================================================
-- OPS-SALE-ROOT-1: Branch POS Account Mappings + RPC Patch
-- Purpose: Remove hardcoded COA codes from complete_pos_sale_atomic
-- =====================================================

-- =====================================================
-- A1) Create enum for account roles
-- =====================================================
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'pos_account_role') THEN
    CREATE TYPE public.pos_account_role AS ENUM (
      'SALES_REVENUE',
      'VAT_PAYABLE',
      'ACCOUNTS_RECEIVABLE',
      'INVENTORY',
...
-- =====================================================
-- A2) Create mapping table
-- =====================================================
CREATE TABLE IF NOT EXISTS public.branch_pos_account_mappings (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  branch_id uuid NOT NULL REFERENCES public.branches(id) ON DELETE CASCADE,
  role pos_account_role NOT NULL,
  account_id uuid NOT NULL REFERENCES public.chart_of_accounts(id),
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT branch_pos_account_mappings_unique UNIQUE(branch_id, role)
);

-- Add index for fast lookup
CREATE INDEX IF NOT EXISTS idx_branch_pos_account_mappings_branch 
  ON public.branch_pos_account_mappings(branch_id);

-- =====================================================
-- A3) Seed mappings for all active branches
-- =====================================================
DO $$
DECLARE
  v_branch record;
...
BEGIN
  ...
  SELECT id INTO v_sales_id FROM public.chart_of_accounts WHERE account_code = '4101' AND is_active = true;
  SELECT id INTO v_vat_id FROM public.chart_of_accounts WHERE account_code = '2104' AND is_active = true;
  SELECT id INTO v_ar_id FROM public.chart_of_accounts WHERE account_code = '1102' AND is_active = true;
  SELECT id INTO v_inventory_id FROM public.chart_of_accounts WHERE account_code = '1103' AND is_active = true;
  SELECT id INTO v_cogs_id FROM public.chart_of_accounts WHERE account_code = '5101' AND is_active = true;
  SELECT id INTO v_cash_id FROM public.chart_of_accounts WHERE account_code = '110101' AND is_active = true;
  SELECT id INTO v_bank_id FROM public.chart_of_accounts WHERE account_code = '110104' AND is_active = true;

  -- Validate all accounts exist
  IF v_sales_id IS NULL THEN
    RAISE EXCEPTION 'COA_MISSING: Sales account 4101 not found or inactive';
  END IF;
  IF v_vat_id IS NULL THEN
    RAISE EXCEPTION 'COA_MISSING: VAT account 2104 not found or inactive';
  END IF;
  IF v_ar_id IS NULL THEN
    RAISE EXCEPTION 'COA_MISSING: AR account 1102 not found or inactive';
  END IF;
  IF v_inventory_id IS NULL THEN
    RAISE EXCEPTION 'COA_MISSING: Inventory account 1103 not found or inactive';
  END IF;
  IF v_cogs_id IS NULL THEN
    RAISE EXCEPTION 'COA_MISSING: COGS account 5101 not found or inactive';
  END IF;
  IF v_cash_id IS NULL THEN
    RAISE EXCEPTION 'COA_MISSING: Cash account 110101 not found or inactive';
  END IF;
  IF v_bank_id IS NULL THEN
    RAISE EXCEPTION 'COA_MISSING: Bank account 110104 not found or inactive';
  END IF;

  -- Seed for each branch
  FOR v_branch IN SELECT id FROM public.branches WHERE is_active = true LOOP
    -- SALES_REVENUE
    INSERT INTO public.branch_pos_account_mappings (branch_id, role, account_id)
    VALUES (v_branch.id, 'SALES_REVENUE', v_sales_id)
    ON CONFLICT (branch_id, role) DO NOTHING;
    
    -- VAT_PAYABLE
    INSERT INTO public.branch_pos_account_mappings (branch_id, role, account_id)
    VALUES (v_branch.id, 'VAT_PAYABLE', v_vat_id)
    ON CONFLICT (branch_id, role) DO NOTHING;
    
    -- ACCOUNTS_RECEIVABLE
    INSERT INTO public.branch_pos_account_mappings (branch_id, role, account_id)
    VALUES (v_branch.id, 'ACCOUNTS_RECEIVABLE', v_ar_id)
    ON CONFLICT (branch_id, role) DO NOTHING;
    
    -- INVENTORY
    INSERT INTO public.branch_pos_account_mappings (branch_id, role, account_id)
    VALUES (v_branch.id, 'INVENTORY', v_inventory_id)
    ON CONFLICT (branch_id, role) DO NOTHING;
    
    -- COGS
    INSERT INTO public.branch_pos_account_mappings (branch_id, role, account_id)
    VALUES (v_branch.id, 'COGS', v_cogs_id)
    ON CONFLICT (branch_id, role) DO NOTHING;
    
    -- CASH
    INSERT INTO public.branch_pos_account_mappings (branch_id, role, account_id)
    VALUES (v_branch.id, 'CASH', v_cash_id)
    ON CONFLICT (branch_id, role) DO NOTHING;
    
    -- BANK
    INSERT INTO public.branch_pos_account_mappings (branch_id, role, account_id)
    VALUES (v_branch.id, 'BANK', v_bank_id)
    ON CONFLICT (branch_id, role) DO NOTHING;
  END LOOP;
END $$;

-- =====================================================
-- A4) Enable RLS with strict policies
-- =====================================================
ALTER TABLE public.branch_pos_account_mappings ENABLE ROW LEVEL SECURITY;

-- Admin-only SELECT
CREATE POLICY "branch_pos_mappings_select_admin"
  ON public.branch_pos_account_mappings
  FOR SELECT
  TO authenticated
  USING (has_role(auth.uid(), 'admin'::app_role));

-- Admin-only INSERT
CREATE POLICY "branch_pos_mappings_insert_admin"
  ON public.branch_pos_account_mappings
  FOR INSERT
  TO authenticated
...
-- Admin-only UPDATE
CREATE POLICY "branch_pos_mappings_update_admin"
  ON public.branch_pos_account_mappings
  FOR UPDATE
  TO authenticated
  USING (has_role(auth.uid(), 'admin'::app_role))
...
-- Admin-only DELETE
CREATE POLICY "branch_pos_mappings_delete_admin"
  ON public.branch_pos_account_mappings
  FOR DELETE
  TO authenticated
  USING (has_role(auth.uid(), 'admin'::app_role));
...
-- =====================================================
-- B) PATCH RPC: complete_pos_sale_atomic uses mapping
-- =====================================================
...
BEGIN
  ...
  -- Override bank account if explicitly provided (for specific bank selection)
  IF v_bank_account_code IS NOT NULL AND v_bank_account_code != '' THEN
    SELECT id INTO v_bank_account_id 
    FROM chart_of_accounts 
    WHERE account_code = v_bank_account_code AND is_active = true;
    
    IF v_bank_account_id IS NULL THEN
      RAISE EXCEPTION 'ACCOUNT_NOT_FOUND: Specified bank account % not found or inactive', v_bank_account_code;
    END IF;
  END IF;
  ...
  -- =====================================================
  -- STEP 7: Get branch code and generate sale code
  -- =====================================================
  ...
  -- =====================================================
  -- STEP 9: Create sale_items with distributed discount
  -- =====================================================
