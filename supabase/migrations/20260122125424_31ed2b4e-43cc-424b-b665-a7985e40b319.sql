-- Stage-2B FIX: Correct purchase_type default value to 'general'
...
BEGIN
    ...
    IF (v_idempotency_check->>'status') = 'succeeded' THEN
        RETURN jsonb_build_object(
            ...
            'meta', jsonb_build_object('workflowType', v_workflow_type, 'clientRequestId', v_client_request_id)
        );
    END IF;
    ...
    -- purchase_type must be 'general' or 'import'
    v_purchase_type := COALESCE(p_payload->'invoice'->>'purchase_type', 'general');
    IF v_purchase_type NOT IN ('general', 'import') THEN
        v_purchase_type := 'general';
    END IF;
    ...
    SELECT supplier_name, account_id INTO v_supplier_name, v_supplier_account_id FROM suppliers WHERE id = v_supplier_id;
    SELECT id INTO v_inventory_account_id FROM chart_of_accounts WHERE account_code = '1137' LIMIT 1;
    SELECT id INTO v_vat_account_id FROM chart_of_accounts WHERE account_code = '2202' LIMIT 1;
    IF v_supplier_account_id IS NULL THEN
        SELECT id INTO v_supplier_account_id FROM chart_of_accounts WHERE account_code = '21010001' LIMIT 1;
    END IF;

    IF v_inventory_account_id IS NULL THEN
        PERFORM core_workflow_failed(v_client_request_id::TEXT, 'CONFIG_ERROR', 'Inventory account 1137 not found');
        RETURN jsonb_build_object('success', false, 'error_code', 'CONFIG_ERROR', 'error', 'Inventory account 1137 not found');
    END IF;
    IF v_supplier_account_id IS NULL THEN
        PERFORM core_workflow_failed(v_client_request_id::TEXT, 'CONFIG_ERROR', 'Supplier AP account not found');
        RETURN jsonb_build_object('success', false, 'error_code', 'CONFIG_ERROR', 'error', 'Supplier AP account not found');
    END IF;
    ...
    IF v_tax_amount > 0 AND v_vat_account_id IS NOT NULL THEN
        INSERT INTO journal_entry_lines (journal_entry_id, account_id, debit_amount, credit_amount, description)
        VALUES (v_je_id, v_vat_account_id, v_tax_amount, 0, 'VAT Purchases - ' || v_invoice_number);
    END IF;
    ...
    IF NOT EXISTS (
        SELECT 1 FROM journal_entry_lines WHERE journal_entry_id = v_je_id
        GROUP BY journal_entry_id HAVING SUM(debit_amount) = SUM(credit_amount) AND SUM(debit_amount) = v_total_amount
    ) THEN
        RAISE EXCEPTION 'PI_ERR:UNBALANCED_JE:Journal entry is not balanced';
    END IF;

    -- Insert Invoice
    INSERT INTO public.invoices (id, invoice_number, invoice_type, purchase_type, supplier_id, branch_id, invoice_date, due_date, notes, subtotal, tax_amount, total_amount, paid_amount, remaining_amount, status, journal_entry_id, created_by, created_at)
    VALUES (v_invoice_id, v_invoice_number, 'purchase', v_purchase_type, v_supplier_id, v_branch_id, v_invoice_date, v_due_date, v_notes, v_subtotal, v_tax_amount, v_total_amount, 0, v_total_amount, 'posted', v_je_id, v_created_by, NOW());
    ...
    LOOP
        ...
        INSERT INTO public.purchase_invoice_lines (invoice_id, line_number, product_id, product_code, description, quantity, unit_price, discount_amount, subtotal, tax_rate, tax_amount, total_amount, item_type, gl_account_id, cost_entry_id, warehouse_id, notes)
        VALUES (v_invoice_id, v_line_number, (v_item->>'item_id')::UUID, v_item->>'item_code', COALESCE(v_item->>'description', 'Item ' || v_line_number), v_line_qty, v_line_unit_price, v_line_discount, v_line_subtotal, v_line_tax_rate * 100, v_line_tax, v_line_total, COALESCE(v_item->>'item_type', 'jewelry'), (v_item->>'gl_account_id')::UUID, (v_item->>'cost_entry_id')::UUID, (v_item->>'warehouse_id')::UUID, v_item->>'line_notes');
    END LOOP;

    v_result := jsonb_build_object(
        'success', true, 'invoiceId', v_invoice_id, 'invoiceNumber', v_invoice_number, 'journalEntryId', v_je_id, 'journalEntryNumber', v_je_number, 'status', 'posted',
        ...
        'meta', jsonb_build_object('workflowType', v_workflow_type, 'clientRequestId', v_client_request_id, 'payloadHash', v_payload_hash)
    );
