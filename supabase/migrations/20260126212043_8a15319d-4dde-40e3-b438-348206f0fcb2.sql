-- P6-4 CLOSEOUT: Audit Logging + View Drift Refinement (WITHOUT metadata backfill)
-- The backfill will be done separately via direct insert to avoid trigger conflict
-- =====================================================

-- B1) Fix RLS policy for audit_events to allow SECURITY DEFINER inserts
...
-- B4) Refine v_returns_hub drift logic to exclude voided/cancelled from drift checks
DROP VIEW IF EXISTS public.v_returns_hub;
...
invoice_line_counts AS (
  SELECT inv.id AS invoice_id, inv.invoice_number, COUNT(pil.id) AS line_count
  FROM invoices inv
...
mirrors AS (
  SELECT ilc.invoice_number AS return_number, true AS mirror_exists
