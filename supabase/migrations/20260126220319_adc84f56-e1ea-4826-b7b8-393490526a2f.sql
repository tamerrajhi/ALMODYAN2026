-- P6-6X ROOT FIX: unblock void for posted JEs (Unique mirror + General aggregation)

-- 1) Fix update_linked_invoice_on_return_change: exclude voided returns same as cancelled
...
BEGIN
    ...
    -- Calculate total returns for the original invoice (excluding cancelled/voided)
    ...
    -- Recalculate returned_qty on original invoice lines (exclude cancelled/voided)
    ...
    SET returned_qty = (
        ...
        WHERE ri.linked_invoice_id = linked_inv_id
          ...
          AND ri.status NOT IN ('cancelled', 'voided')
          AND (
...
-- 2) Fix void_purchase_return_atomic: for UNIQUE path, mirror invoice should be voided (not cancelled)
--    to pass invoices_posted_lock when JE is posted. Also keep void metadata.
...
BEGIN
  ...
  IF v_is_unique THEN
    ...
    -- Mirror invoice: VOID it (not cancel) to pass POSTED_LOCKED
  ...
  -- Should not reach
  RETURN jsonb_build_object('success', false, 'error_code', 'UNKNOWN', 'error', 'Unexpected state');

EXCEPTION WHEN OTHERS THEN
  -- Ensure workflow is marked failed
