-- ============================================================
-- PV-4.1 HOTFIX: Fix idempotency gating + JE reversal safety
-- ============================================================

-- 1) Safer reversal helper (no assumptions about extra columns)
...
BEGIN
  ...
  IF NOT FOUND THEN
    RETURN jsonb_build_object('success', false, 'error_code','NOT_FOUND','error', 'Original journal entry not found');
  END IF;

  IF COALESCE(v_original_je.is_reversed,false) THEN
    RETURN jsonb_build_object('success', true, 'alreadyReversed', true, 'originalJournalEntryId', p_original_je_id);
  END IF;

  -- Prefer official generator
  IF EXISTS (
    SELECT 1 FROM pg_proc p
    JOIN pg_namespace n ON n.oid=p.pronamespace
    WHERE n.nspname='public' AND p.proname='generate_journal_entry_number'
  ) THEN
    SELECT public.generate_journal_entry_number() INTO v_reversal_entry_number;
  ELSE
    -- fallback if missing
    v_reversal_entry_number := 'REV-' || to_char(now(),'YYYYMMDD-HH24MISS');
  END IF;
...
GRANT EXECUTE ON FUNCTION public.reverse_journal_entry_atomic(uuid, uuid, text, text, uuid, text) TO authenticated, service_role;

-- 2) Fix VOID RPC: JSON idempotency gate + cached_result
...
BEGIN
  ...
  v_void_reason := COALESCE(NULLIF(p_payload->>'void_reason',''), 'Voided by user');
  v_void_date := COALESCE(NULLIF(p_payload->>'void_date','')::date, CURRENT_DATE);
  v_created_by := COALESCE(NULLIF(p_payload->>'created_by',''), 'system');

  v_begin := public.begin_workflow_request(v_client_request_id,'payment_voucher_void_atomic',p_payload);
  ...
  IF v_payment.journal_entry_id IS NOT NULL THEN
    ...
    IF COALESCE((v_reversal_result->>'success')::boolean,false) = false THEN
...
GRANT EXECUTE ON FUNCTION public.payment_voucher_void_atomic(jsonb) TO authenticated, service_role;

-- 3) Fix UPDATE RPC: JSON idempotency gate + safe JE generation + avoid non-existent columns
