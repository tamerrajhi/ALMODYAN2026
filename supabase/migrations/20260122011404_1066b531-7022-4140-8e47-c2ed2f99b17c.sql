-- PV3 Stage-1: Fix canonical writer signature mismatches
-- Changes: 
--   1. core_workflow_failed: 4 args → 3 args (merge workflow_type into error_message)
--   2. core_workflow_success: arg2 text → uuid (use v_payment_id as entity_id)
...
BEGIN
  ...
  IF v_payment_type = 'payment' 
    ...
    -- FIX: 3 args instead of 4 (merge workflow_type into error_message)
  ...
  -- FIX: arg2 is entity_id (uuid) not workflow_type (text)
