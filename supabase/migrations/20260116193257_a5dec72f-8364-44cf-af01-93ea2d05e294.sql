-- Fix column name: item_id instead of jewelry_item_id
...
BEGIN
  ...
  -- ===================
  -- GUARDRAIL: Enforce "Posted = JE" when total_cost > 0
  -- ===================
  ...
  -- ===================
  -- INSERT TRANSFER ITEMS (FIXED: item_id not jewelry_item_id)
  -- ===================
  ...
  IF v_total_cost > 0 THEN
    ...
    -- Get JE sequence from branch_code_sequences (NO fallback)
    SELECT prefix, padding, next_value
    ...
    -- Line 2: Credit source branch inventory
    INSERT INTO public.journal_entry_lines (
      journal_entry_id,
      account_id,
      debit_amount,
