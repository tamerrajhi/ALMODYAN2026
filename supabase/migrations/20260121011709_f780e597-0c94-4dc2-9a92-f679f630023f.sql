-- Enhanced void_purchase_return_atomic with full idempotency + correct inventory restoration
...
BEGIN
  -- ============================
  -- 1. Parse & Validate Input (Support both old and new payload formats)
  -- ============================
  ...
  -- ============================
  -- 2. Advisory Lock (per-request serialization)
  -- ============================
  ...
  -- 3. Idempotency Gate
  -- ============================
  v_workflow_result := public.begin_workflow_request(v_client_request_id, 'void_purchase_return', p_payload);
  ...
  -- ============================
  -- 5. Status Validation (Idempotent for already voided)
  -- ============================
  ...
  IF v_return_rec.status NOT IN ('confirmed', 'posted') THEN
    ...
    RETURN jsonb_build_object(
      ...
      'error', 'Cannot void return with status: ' || v_return_rec.status || '. Only confirmed/posted returns can be voided.'
    );
  END IF;
  ...
  -- ============================
  -- 7. Restore Jewelry Items + Create Reversal Movements
  -- ============================
  FOR v_item IN
  ...
  LOOP
    ...
    IF NOT EXISTS (
      ...
      WHERE id = v_item.jewelry_item_id AND sale_status = 'returned'
    ) THEN
      -- Item is not in 'returned' state - might have been modified manually
      -- Log warning but continue (for resilience)
      NULL;
    END IF;

    -- Restore jewelry item to available + original branch
    ...
    INSERT INTO public.item_movements (
      ...
      v_voided_by,
      NOW(),
      'إلغاء مرتجع: ' || v_return_rec.return_number || ' - ' || v_void_reason,
      v_item.unit_price,
      v_reversal_je_id
    );
  END LOOP;
  ...
  -- ============================
  -- 8. Update Purchase Return Status
  -- ============================
  ...
  -- ============================
  -- 9. Build Result & Complete Workflow
  -- ============================
  v_result := jsonb_build_object(
    'success', true,
    'purchase_return_id', v_return_rec.id,
