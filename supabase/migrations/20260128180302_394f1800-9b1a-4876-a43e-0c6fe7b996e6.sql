-- ============================================================
-- P-PURCH-GENERAL-RETURNS-FIX | Fix column mismatch: created_by → processed_by
-- Only patching the INSERT into purchase_returns
-- ============================================================
...
BEGIN
  ...
  SELECT id INTO v_vat_account_id FROM public.chart_of_accounts WHERE account_code = '1502' AND is_active = true LIMIT 1;
  
  IF v_ap_account_id IS NULL OR v_inventory_account_id IS NULL THEN
    RETURN jsonb_build_object('success', false, 'error_code', 'CONFIG', 'error', 'Required GL accounts not found');
  END IF;
  
  -- ============================================================
  -- STEP 3: Begin idempotent workflow
  -- ============================================================
  IF NOT v_dry_run THEN
    v_gate := public.pos_begin_request(v_client_request_id, 'purchase_return_general_create_atomic', p_payload);
    
    IF v_gate->>'status' = 'completed' THEN
      RETURN jsonb_build_object(
        'success', true,
        'cached', true,
        'idempotent', true,
        'returnId', v_gate->'result'->>'returnId',
        'returnNumber', v_gate->'result'->>'returnNumber',
        'journalEntryId', v_gate->'result'->>'journalEntryId',
        'journalEntryNumber', v_gate->'result'->>'journalEntryNumber'
      );
    END IF;
    
    IF v_gate->>'status' = 'processing' THEN
      NULL;
    END IF;
    ...
    IF v_gate->>'status' = 'failed' THEN
      RETURN jsonb_build_object(
        ...
        'error', 'Previous attempt failed: ' || COALESCE(v_gate->>'error', 'unknown'),
        'cached', true
      );
    END IF;
  END IF;
  ...
  -- ============================================================
  -- STEP 4: Pre-flight validation: Check all items
  -- ============================================================
  FOR v_item IN SELECT * FROM jsonb_array_elements(v_items)
  LOOP
    ...
    IF v_invoice_line_id IS NOT NULL THEN
      SELECT pil.product_id, pil.product_code, pil.line_number, pil.quantity,
  ...
  LOOP
    ...
    INSERT INTO public.purchase_return_lines (
      return_id, invoice_line_id, line_number, product_id, product_code,
      description, item_type, quantity, unit_price, discount_amount,
      tax_rate, tax_amount, line_total, reason
    ) VALUES (
      v_return_id, v_invoice_line_id, v_line_number, v_product_id, v_product_code,
      v_description, v_item_type, v_quantity, v_unit_price, v_discount_amount,
      v_vat_rate_decimal, v_line_tax, v_line_total, v_item->>'reason'
    );
    v_lines_inserted := v_lines_inserted + 1;
  END LOOP;
  ...
  PERFORM public.pos_succeed_request(v_client_request_id, v_result);
  
  RETURN v_result;

EXCEPTION WHEN OTHERS THEN
  IF v_client_request_id IS NOT NULL AND NOT COALESCE(v_dry_run, false) THEN
    BEGIN
...
COMMENT ON FUNCTION public.complete_purchase_return_general_atomic(jsonb) IS
  'P-PURCH-GENERAL-RETURNS-FIX: Fixed column mismatch (created_by → processed_by). Security hardened per D2-5.3 B1.';