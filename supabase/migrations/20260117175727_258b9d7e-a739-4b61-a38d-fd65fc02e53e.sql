-- PV-1.1 HOTFIX: Fix payment_voucher_atomic RPC
-- Fixes: JSON return handling, UUID parsing, journal_entries schema, stable_payload_hash

CREATE OR REPLACE FUNCTION public.payment_voucher_atomic(p_payload jsonb)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, public, extensions
AS $function$
DECLARE
  v_client_request_id uuid;
  v_begin jsonb;
  v_workflow_status text;
  v_payload_hash text;
  v_requested_by uuid;
  v_created_by text;
  
  -- Payment fields
  v_payment_id uuid;
  v_payment_number text;
  v_payment_type text;
  v_payment_date date;
  v_amount numeric;
  v_payment_method text;
  v_supplier_id uuid;
  v_customer_id uuid;
  v_invoice_id uuid;
  v_bank_account text;
  v_check_number text;
  v_currency text;
  v_exchange_rate numeric;
  v_branch_id uuid;
  v_notes text;
  v_status text;
  
  -- Journal entry fields
  v_journal_entry_id uuid;
  v_journal_entry_number text;
  v_entry_date date;
  v_je_description text;
  v_reference_type text;
  
  -- Calculated fields
  v_total_debit numeric := 0;
  v_total_credit numeric := 0;
  v_line record;
  
  v_result jsonb;
BEGIN
  -- ===========================================
  -- A) Parse and validate client_request_id
  -- ===========================================
  IF p_payload->>'client_request_id' IS NULL OR p_payload->>'client_request_id' = '' THEN
    RETURN jsonb_build_object(
      'success', false,
      'error_code', 'VALIDATION',
      'error', 'client_request_id is required'
    );
  END IF;
  
  BEGIN
    v_client_request_id := (p_payload->>'client_request_id')::uuid;
  EXCEPTION WHEN OTHERS THEN
    RETURN jsonb_build_object(
      'success', false,
      'error_code', 'VALIDATION',
      'error', 'client_request_id must be a valid UUID'
    );
  END;

  -- ===========================================
  -- B) Canonical idempotency gate (JSON return)
  -- ===========================================
  v_begin := public.begin_workflow_request(v_client_request_id, 'payment_voucher_atomic', p_payload);
  v_workflow_status := v_begin->>'status';
  
  IF v_workflow_status = 'succeeded' THEN
    -- Return cached result directly
    RETURN v_begin->'cached_result';
  ELSIF v_workflow_status = 'conflict' THEN
    RETURN jsonb_build_object(
      'success', false,
      'error_code', 'IDEMPOTENCY_CONFLICT',
      'error', COALESCE(v_begin->>'error_message', 'client_request_id reused with different payload')
    );
  ELSIF v_workflow_status = 'in_progress' THEN
    RETURN jsonb_build_object(
      'success', false,
      'error_code', 'IN_PROGRESS',
      'error', COALESCE(v_begin->>'error_message', 'request is already being processed')
    );
  END IF;
  -- Continue for 'ok' or 'retry'
  
  -- ===========================================
  -- C) Calculate stable payload hash
  -- ===========================================
  v_payload_hash := public.stable_payload_hash(p_payload);
  
  -- ===========================================
  -- D) Safe UUID parsing with NULLIF
  -- ===========================================
  v_requested_by := NULLIF(p_payload->>'requested_by', '')::uuid;
  v_created_by := NULLIF(p_payload->>'created_by', '');
  
  v_payment_type := p_payload->'payment'->>'payment_type';
  v_payment_date := COALESCE(
    NULLIF(p_payload->'payment'->>'payment_date', '')::date,
    CURRENT_DATE
  );
  v_amount := (p_payload->'payment'->>'amount')::numeric;
  v_payment_method := NULLIF(p_payload->'payment'->>'payment_method', '');
  v_supplier_id := NULLIF(p_payload->'payment'->>'supplier_id', '')::uuid;
  v_customer_id := NULLIF(p_payload->'payment'->>'customer_id', '')::uuid;
  v_invoice_id := NULLIF(p_payload->'payment'->>'invoice_id', '')::uuid;
  v_bank_account := NULLIF(p_payload->'payment'->>'bank_account', '');
  v_check_number := NULLIF(p_payload->'payment'->>'check_number', '');
  v_currency := COALESCE(NULLIF(p_payload->'payment'->>'currency', ''), 'SAR');
  v_exchange_rate := COALESCE((p_payload->'payment'->>'exchange_rate')::numeric, 1);
  v_branch_id := NULLIF(p_payload->'payment'->>'branch_id', '')::uuid;
  v_notes := NULLIF(p_payload->'payment'->>'notes', '');
  v_status := COALESCE(NULLIF(p_payload->'payment'->>'status', ''), 'posted');
  
  -- Journal fields
  v_entry_date := COALESCE(
    NULLIF(p_payload->'journal'->>'entry_date', '')::date,
    v_payment_date
  );
  v_je_description := NULLIF(p_payload->'journal'->>'description', '');
  v_reference_type := COALESCE(NULLIF(p_payload->'journal'->>'reference_type', ''), 'payment');

  -- ===========================================
  -- E) Validate payload
  -- ===========================================
  IF v_payment_type IS NULL OR v_payment_type = '' THEN
    PERFORM public.core_workflow_failed(v_client_request_id, 'VALIDATION', 'payment_type is required');
    RETURN jsonb_build_object(
      'success', false,
      'error_code', 'VALIDATION',
      'error', 'payment_type is required'
    );
  END IF;
  
  IF v_amount IS NULL OR v_amount <= 0 THEN
    PERFORM public.core_workflow_failed(v_client_request_id, 'VALIDATION', 'amount must be greater than 0');
    RETURN jsonb_build_object(
      'success', false,
      'error_code', 'VALIDATION',
      'error', 'amount must be greater than 0'
    );
  END IF;
  
  IF p_payload->'lines' IS NULL OR jsonb_array_length(p_payload->'lines') = 0 THEN
    PERFORM public.core_workflow_failed(v_client_request_id, 'VALIDATION', 'lines array is required and must not be empty');
    RETURN jsonb_build_object(
      'success', false,
      'error_code', 'VALIDATION',
      'error', 'lines array is required and must not be empty'
    );
  END IF;
  
  -- Calculate totals and validate lines
  FOR v_line IN SELECT * FROM jsonb_array_elements(p_payload->'lines')
  LOOP
    IF v_line.value->>'account_id' IS NULL OR v_line.value->>'account_id' = '' THEN
      PERFORM public.core_workflow_failed(v_client_request_id, 'VALIDATION', 'All lines must have account_id');
      RETURN jsonb_build_object(
        'success', false,
        'error_code', 'VALIDATION',
        'error', 'All lines must have account_id'
      );
    END IF;
    
    v_total_debit := v_total_debit + COALESCE((v_line.value->>'debit_amount')::numeric, 0);
    v_total_credit := v_total_credit + COALESCE((v_line.value->>'credit_amount')::numeric, 0);
  END LOOP;
  
  -- Validate balanced journal entry
  IF v_total_debit <= 0 OR v_total_credit <= 0 THEN
    PERFORM public.core_workflow_failed(v_client_request_id, 'VALIDATION', 'Journal entry must have both debit and credit amounts > 0');
    RETURN jsonb_build_object(
      'success', false,
      'error_code', 'VALIDATION',
      'error', 'Journal entry must have both debit and credit amounts > 0'
    );
  END IF;
  
  IF v_total_debit <> v_total_credit THEN
    PERFORM public.core_workflow_failed(v_client_request_id, 'VALIDATION', format('Journal entry not balanced: debit=%s credit=%s', v_total_debit, v_total_credit));
    RETURN jsonb_build_object(
      'success', false,
      'error_code', 'VALIDATION',
      'error', format('Journal entry not balanced: debit=%s credit=%s', v_total_debit, v_total_credit)
    );
  END IF;

  -- ===========================================
  -- F) Advisory lock + generate payment number
  -- ===========================================
  PERFORM pg_advisory_xact_lock(hashtext(v_payment_type || '-' || to_char(CURRENT_DATE, 'YYYYMMDD')));
  
  SELECT public.generate_payment_number(v_payment_type) INTO v_payment_number;
  
  -- ===========================================
  -- G) Generate journal entry number
  -- ===========================================
  SELECT public.generate_journal_entry_number() INTO v_journal_entry_number;
  
  -- ===========================================
  -- H) Insert payment record
  -- ===========================================
  v_payment_id := gen_random_uuid();
  
  INSERT INTO public.payments (
    id,
    payment_number,
    payment_type,
    payment_date,
    amount,
    payment_method,
    supplier_id,
    customer_id,
    invoice_id,
    bank_account,
    check_number,
    currency,
    exchange_rate,
    branch_id,
    notes,
    status,
    created_by,
    created_at,
    updated_at
  ) VALUES (
    v_payment_id,
    v_payment_number,
    v_payment_type,
    v_payment_date,
    v_amount,
    v_payment_method,
    v_supplier_id,
    v_customer_id,
    v_invoice_id,
    v_bank_account,
    v_check_number,
    v_currency,
    v_exchange_rate,
    v_branch_id,
    v_notes,
    v_status,
    v_created_by,
    now(),
    now()
  );
  
  -- ===========================================
  -- I) Insert journal entry (NO status column!)
  -- ===========================================
  v_journal_entry_id := gen_random_uuid();
  
  INSERT INTO public.journal_entries (
    id,
    entry_number,
    entry_date,
    description,
    reference_type,
    reference_id,
    branch_id,
    is_posted,
    posted_at,
    posted_by,
    total_debit,
    total_credit,
    created_by,
    created_at,
    updated_at
  ) VALUES (
    v_journal_entry_id,
    v_journal_entry_number,
    v_entry_date,
    COALESCE(v_je_description, 'Payment Voucher ' || v_payment_number),
    v_reference_type,
    v_payment_id,
    v_branch_id,
    true,
    now(),
    COALESCE(v_created_by, v_requested_by::text),
    v_total_debit,
    v_total_credit,
    v_created_by,
    now(),
    now()
  );
  
  -- ===========================================
  -- J) Insert journal entry lines
  -- ===========================================
  INSERT INTO public.journal_entry_lines (
    id,
    journal_entry_id,
    account_id,
    debit_amount,
    credit_amount,
    description,
    created_at
  )
  SELECT
    gen_random_uuid(),
    v_journal_entry_id,
    (line->>'account_id')::uuid,
    COALESCE((line->>'debit_amount')::numeric, 0),
    COALESCE((line->>'credit_amount')::numeric, 0),
    NULLIF(line->>'description', ''),
    now()
  FROM jsonb_array_elements(p_payload->'lines') AS line;
  
  -- ===========================================
  -- K) Link payment to journal entry
  -- ===========================================
  UPDATE public.payments
  SET journal_entry_id = v_journal_entry_id
  WHERE id = v_payment_id;
  
  -- ===========================================
  -- L) Build result and mark success
  -- ===========================================
  v_result := jsonb_build_object(
    'success', true,
    'paymentId', v_payment_id,
    'paymentNumber', v_payment_number,
    'journalEntryId', v_journal_entry_id,
    'journalEntryNumber', v_journal_entry_number,
    'meta', jsonb_build_object(
      'workflowType', 'payment_voucher_atomic',
      'clientRequestId', v_client_request_id,
      'payloadHash', v_payload_hash
    )
  );
  
  PERFORM public.core_workflow_success(v_client_request_id, v_payment_id, v_result);
  
  RETURN v_result;

EXCEPTION WHEN OTHERS THEN
  -- Mark workflow as failed
  PERFORM public.core_workflow_failed(v_client_request_id, SQLSTATE, SQLERRM);
  
  RETURN jsonb_build_object(
    'success', false,
    'error_code', 'DB_ERROR',
    'error', SQLERRM
  );
END;
$function$;

-- Ensure grants
GRANT EXECUTE ON FUNCTION public.payment_voucher_atomic(jsonb) TO authenticated, service_role;