-- Fix idempotency_smoke_atomic to handle actual begin_workflow_request return values
-- The function returns: 'processing' (new/in-progress), 'succeeded' (cached), 'failed'
...
BEGIN
  ...
  -- Actual values: 'processing' (new), 'succeeded' (cached), 'failed'
  -- ===========================================
  
  -- SUCCEEDED: Previously succeeded, return cached result
  IF v_begin_status = 'succeeded' THEN
    -- The cached result is the whole v_begin_result or v_begin_result itself
    RETURN v_begin_result;
  END IF;
  
  -- FAILED: Previous attempt failed
  IF v_begin_status = 'failed' THEN
    RETURN jsonb_build_object(
      'success', false,
      'error_code', COALESCE(v_begin_result->>'error_code', 'PREVIOUS_FAILURE'),
      'error', COALESCE(v_begin_result->>'error_message', 'Previous attempt failed')
    );
  END IF;
  
  -- PROCESSING: Either new request or already in progress
  -- For new request, we proceed. For in-progress by another process, this is tricky.
  -- The current begin_workflow_request doesn't distinguish between "I just created it" 
  -- vs "someone else is processing". We'll proceed with logic.
  IF v_begin_status != 'processing' THEN
    RETURN jsonb_build_object(
      'success', false,
