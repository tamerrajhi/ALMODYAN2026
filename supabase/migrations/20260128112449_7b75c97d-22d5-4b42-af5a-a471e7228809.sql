-- ============================================
-- D2-5.1: Harden void_purchase_return_atomic for General
-- Remove legacy invoice mirror updates for general returns
-- ============================================
...
DECLARE
  ...
  v_invoice_id uuid;  -- Legacy fallback only
  v_voided_by text;
...
BEGIN
  ...
  -- ============================
  -- 5. Legacy Fallback: invoice_id for old general returns (only if not found above)
  -- ============================
  IF v_return_rec.id IS NULL AND v_invoice_id IS NOT NULL THEN
    -- Try to find matching canonical return by journal_entry_id linkage
    SELECT 
      pr.id, 
      pr.return_number, 
      pr.journal_entry_id, 
  ...
  IF v_return_type = 'unique' THEN
    ...
    -- Note: Only unique returns may have invoice mirrors in legacy systems
  ...
  -- Legacy mirrors are locked/cancelled via D2-3/D2-4
  -- ============================
  IF v_return_type = 'general' THEN
    -- Initialize mirror_invoice_id to NULL for general returns
    -- We do NOT update invoices for general returns post-cutover
    v_mirror_invoice_id := NULL;
    ...
    LOOP
      ...
      INSERT INTO public.raw_material_movements (
        ...
        v_item.material_id,
        v_item.product_id,
        'purchase_return',  -- Same type, opposite quantity
        -v_item.quantity,   -- Reverse the quantity (if original was -5, this is +5)
        v_item.unit_cost,
      ...
      SET 
        quantity = rms.quantity - v_item.quantity,  -- Reverse effect
        updated_at = now()
    ...
    -- NOTE: Legacy invoice mirrors for general returns are already cancelled
    -- via D2-4 cleanup. We do NOT touch invoices table for general returns.
  END IF;
...
-- Add comment documenting the hardening
COMMENT ON FUNCTION public.void_purchase_return_atomic(jsonb) IS 
'D2-5.1 Hardened: General returns update only purchase_returns (canonical). 
No invoice updates for general returns post-cutover. 
Unique returns may still update legacy invoice mirrors if they exist.';