-- Fix root cause of SALE_MOVEMENTS_INCOMPLETE:
-- item_movements has a BEFORE trigger (normalize_movement_type) that normalizes movement_type,
-- while complete_pos_sale_atomic was inserting/checking 'SALE' (uppercase).
-- This caused the movement rows to be inserted as 'sale' then the function counted only 'SALE' => 0 => exception => full rollback.
...
BEGIN
  ...
  -- =====================================================
  -- STEP 3: Idempotency check (P2N: enforce hash for succeeded too)
  -- =====================================================
  ...
  BEGIN
    ...
    IF FOUND THEN
      IF v_existing_request.status = 'succeeded' THEN
        -- P2N-FIX: Check payload_hash BEFORE returning idempotent success
        IF v_existing_request.payload_hash IS DISTINCT FROM v_payload_hash THEN
          RAISE EXCEPTION 'CONFLICT_PAYLOAD_MISMATCH: Previous request with same ID had different payload';
        END IF;
        -- Payload matches, safe to return cached result
  ...
  -- P2H-A: Block duplicate items in payload
  IF EXISTS (
    SELECT 1
    FROM (
  ...
  -- P2C: split FOR UPDATE locking from aggregate
  PERFORM 1
...
-- P2N: Enhanced exception handling with explicit CONFLICT_* error codes
EXCEPTION
  ...
  WHEN OTHERS THEN
    ...
    -- P2N-FIX: Return explicit error_code for CONFLICT_* exceptions
    IF SQLERRM LIKE 'CONFLICT_PAYLOAD_MISMATCH:%' THEN
      RETURN jsonb_build_object(
        'success', false,
        'error', SQLERRM,
        'error_code', 'CONFLICT_PAYLOAD_MISMATCH',
        'sqlstate', SQLSTATE
      );
    ELSIF SQLERRM LIKE 'CONFLICT_IN_PROGRESS:%' THEN
      RETURN jsonb_build_object(
        'success', false,
        'error', SQLERRM,
